...
$delldepotURL = "https://vmwaredepot.dell.com/index.xml"
...
if ($help) {
    ...
    write-host "   -dell              : connect to the Dell Online depot"
...
if ($dell) {
    # Connect the V-Front Online depot
    write-host -nonewline "`nConnecting the Dell Online depot ..."
    if ($vftdepot = Add-EsxSoftwaredepot $delldepotURL) {
        write-host -ForegroundColor Green " [OK]"
    } else {
        write-host -ForegroundColor Red "`nFATAL ERROR: Cannot add the Dell Online depot. Please check your internet connectivity and/or proxy settings!`n"
        exit
    }
}
